...
if __name__ == '__main__':
    ...
    stockDataDir = os.environ.get(
    "LOB_GAN_DATA_DIR",
        r"E:\Workbench\UChicago\UChicago_2025MMMLectures\Assignments\Assignment4\references"
    )
    stock = os.environ.get("LOB_GAN_STOCK", "0050")
    if not stockDataDir.endswith(os.sep):
        stockDataDir += os.sep
    ...
    #set up
    set_seed(int(os.environ.get("LOB_GAN_SEED", "307")))
    ...
    if os.environ.get("LOB_GAN_SKIP_PLOTS", "0") == "1":
        print("Skipping plots (LOB_GAN_SKIP_PLOTS=1).")
    else:
        can_plot_dis = (not dis_ret.empty) and ("return" in dis_ret.columns) and (dis_ret["return"].nunique() > 1)

        plt.figure(figsize=(8, 4))
        if can_plot_dis:
            sns.kdeplot(dis_ret["return"].values, linestyle="--", fill=True, label="discriminatedRtn")
        else:
            print("Not enough variance in discriminated returns for KDE; skipping that curve.")

        try:
            if hasattr(ret, "nunique") and ret.nunique() > 1:
                sns.kdeplot(ret.values, label="realRtn")
            else:
                print("Not enough variance in real returns for KDE; skipping that curve.")
        except Exception as e:
            print(f"Skipping real return KDE due to error: {e}")

        plt.legend()
        plt.tight_layout()
        plt.savefig(stock + "_return.png", dpi=150)
        plt.show()
